...
void CreateListF(LinkList &L, int a[], int n)  // 需要背下这个高大上的名字CreateListF（背）
{
    LNode *s;                               // 该指针要作为注射器
    int i;
    L = (LinkList)malloc(sizeof(LNode));    // 创建头结点        第一步：要创建第一个冰激凌，该冰激凌要照顾其他孩子
    L->next = NULL;

    for(i = 0; i < n; i++)                  // （背）
    {
        s = (LNode*)malloc(sizeof(LNode));  // 创建新结点        第二步：生产一个薄饼，并让注射器指向它
...
void CreateListR(LinkList &L, int a[], int n)  // 需要背下这个高大上的名字CreateListR（背）
{
    ...
    L->next = NULL;
    r = L;      // r始终指向链表的尾结点，开始时指向头结点     第二步：设r作为监视器，始终注视着尾结点

    for(i = 0; i < n; i++)
    {
        s = (LinkList)malloc(sizeof(LNode));    // 创建新结点    第三步：生产一个薄饼，并让注射器指向它
