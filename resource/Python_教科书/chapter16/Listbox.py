...
class Application(tk.Frame): #定义GUI应用程序类，派生于Frame类
    ...
    def createWidgets(self): #对象方法：创建子组件
        ...
        self.listboxLeft.grid(row=0, column=0, rowspan=5) #0行0列跨5行
        ...
        self.btnToRight.grid(row=1, column=1)   #1行1列
        self.btnToLeft = tk.Button(self, text='   <   ', command=self.funcToLeft) #创建按钮组件
        self.btnToLeft.grid(row=3, column=1)    #3行1列
    def funcToRight(self): #定义事件处理程序：在右边列表框显示选中的内容
    ...
    def funcToLeft(self): #定义事件处理程序：在左边列表框恢复显示选中的内容
